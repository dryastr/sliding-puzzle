...
func _ready():
	...
	dim_panel = $DimPanel
	win_label.hide() # Hide the win label at the start
	reset_button.hide() # Hide the reset button at the start
	dim_panel.hide() # Hide the dim panel at the start
	start_game()
...
func _process(delta):
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and mouse:
		...
		check_neighbours(rows, cols)
		if tiles == solved:
			win_label.text = "Kamu berhasil!" # Update label text
			win_label.show() # Show the win label
			reset_button.show() # Show the reset button
			dim_panel.show() # Show the dim panel
